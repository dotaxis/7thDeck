...
use std::{
    collections::HashMap, env, fmt::Write, fs::File, path::PathBuf, path::Path, time::Duration
};
use rfd::FileDialog;
use sysinfo::System;
...
fn download_asset(repo: &str, destination: PathBuf) -> Result<PathBuf> {
    ...
    let file_name = exe_asset["name"].as_str().context("Invalid destination file name")?;
...
fn install_7th(game: &SteamGame, exe_path: PathBuf, install_path: &Path, log_file: &str) -> Result<()> {
    ...
    std::fs::copy(toml_path, install_path.join("7thDeck.toml")).context("Failed to copy TOML to install_path")?;
...
fn patch_install(install_path: &Path, game: &SteamGame) -> Result<()> {
    ...
    std::fs::write(&timeout_exe.destination, timeout_exe.contents)
        .with_context(|| format!("Couldn't write {} to {:?}", timeout_exe.name, timeout_exe.destination))?;
...
fn create_shortcuts(install_path: &Path, steam_dir: SteamDir) -> Result<()> {
    ...
    std::fs::write(&shortcut_file.destination, &shortcut_file.contents)
        .with_context(|| format!("Couldn't write {} to {:?}", shortcut_file.name, shortcut_file.destination))?;

    // Icon
    let xdg_cache = xdg::BaseDirectories::new().get_cache_home().context("Couldn't get cache_home")?;
    let logo_png = resource_handler::as_bytes("7th-heaven.png".to_string(), xdg_cache, resource_handler::LOGO_PNG);
    std::fs::write(&logo_png.destination, &logo_png.contents)
        .with_context(|| format!("Couldn't write {} to {:?}", logo_png.name, logo_png.destination))?;
    ...
    match confirm {
        0 => {
            term.clear_last_lines(1)?;
            let desktop_dir = home::home_dir().context("Couldn't get $HOME?")?.join("Desktop");
            println!("{} Adding Desktop shortcut.", console::style("!").yellow());
            let desktop_shortcut_path = desktop_dir.join(&shortcut_file.name);
            std::fs::write(&desktop_shortcut_path, shortcut_file.contents)
                .with_context(|| format!("Couldn't write {} to {:?}", shortcut_file.name, desktop_shortcut_path))?;
        },
        _ => {
            term.clear_last_lines(1)?;
        }
